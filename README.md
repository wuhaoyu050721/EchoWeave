...
<p align="center">
  本地优先、面向 Android 的自定义 AI 角色对话与群聊应用。
</p>

织语是一款为中文角色扮演场景设计的类酒馆应用，基于 Vue 3 与 uni-app 构建。它支持自定义模型接口、角色卡、世界书及多角色群聊，并将聊天记录、角色资料和密钥优先保存在本地设备中。
...
5. 按需在设置中调整流式传输、分段显示、角色状态栏、NSFW、自动同步、回复通知和应用锁。
...
首次运行端到端测试前需要执行 `npx playwright install chromium`。当前自动化测试覆盖模型协议、流式传输、单聊与群聊、角色卡、世界书、状态解析、云同步、加密备份、Android SQLite 与主要界面交互；GitHub Actions 会在推送和拉取请求中运行 Node.js 测试、生产构建、桌面及 Pixel 7 视口 E2E，以及 PHP 服务端集成测试。
