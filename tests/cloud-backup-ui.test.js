...
test('settings exposes account, incremental sync, and encrypted full-backup actions', async () => {
  ...
  assert.match(source, /@click="saveCloudUsername"/)
  ...
  assert.match(source, /sessionBaseUrl && sessionBaseUrl !== baseUrl/)
  ...
  assert.match(source, /uniApi\.showModal/)
...
test('waits for an active sync before replacing services or closing a workspace', async () => {
  ...
  assert.match(prepareCloud, /await this\.stopCloudActivityAndWait\(\)[\s\S]+this\.cloudServices = this\.buildCloudServices\(baseUrl\)/)
