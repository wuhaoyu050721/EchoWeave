...
test('manual sync and full backup actions safely reuse the encrypted sync password after restart', async () => {
  ...
  assert.match(uploadBlock, /onProgress: progress =>/)
  assert.match(uploadBlock, /cloudBackupProgressText\(progress\)/)
  assert.match(uploadBlock, /cloud_backup_completed/)
  assert.match(uploadBlock, /cloud_backup_failed/)
  assert.match(source, /cloudBackupFailureText\(error, fallbackBytes = 0\)/)
