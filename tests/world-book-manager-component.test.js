...
test('world-book manager template includes the complete editable field and interaction contract', async () => {
  ...
  assert.match(source, /:class="\{ 'editor-open': editorOpen \}"/)
  assert.match(source, /\.world-book-manager-sheet\.editor-open\s*\{[^}]*height: min\(92vh, 820px\);/s)
  assert.match(source, /\.manager-editor-scroll\s*\{[^}]*height: 0;[^}]*flex: 1 1 0;/s)
  assert.match(source, /\.manager-editor-actions\s*\{[^}]*background: #fff;[^}]*flex: 0 0 auto;/s)
  assert.match(source, /\.manager-editor-tail\s*\{[^}]*height: 80px;/s)
